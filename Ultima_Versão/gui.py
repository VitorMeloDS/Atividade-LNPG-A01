from  tkinter import Tk, Label, Button, Entry, Toplevel, Radiobutton, IntVar, END, Variable, messagebox
...
def save_button_command(event):
    ...
    if a != '' and b != '' and c != '':
        if len(c) > 4 or len(c) < 4:
            ...
            janela.destroy()
    else: messagebox.showinfo("insira os dados", "Todos os dados são obrigatórios, por favor preencha o fomulário corretamente.")
